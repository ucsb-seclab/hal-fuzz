...
for filepath in $input_dir/*; do
    ...
    mkdir "$out_dir/$filename" | exit -1
    python3 -m hal_fuzz.harness --state-out="$out_dir/$filename" -l $INSTR_LIMIT --dump-mmio-states -m -n --native-lib=$native_lib_path -c $config $filepath || exit -1
done
